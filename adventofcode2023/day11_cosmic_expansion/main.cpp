...
long calc_dist(int i1, int j1, int i2, int j2, const vector<int> &empty_rows, const vector<int> &empty_cols) {
    int EXP_COEF = 1000000;
    int min_i=min(i1,i2);
    int max_i=max(i1,i2);
    int min_j=min(j1,j2);
    int max_j=max(j1,j2);
    ...
    return max_i - min_i + i_exp_cnt*(EXP_COEF-1) + max_j - min_j + j_exp_cnt*(EXP_COEF-1);
}
...
int main()
{
    ...
    for (int i=0;i<galaxies.size();i++){
        ...
        for (int j=0;j<i;j++) {
            ...
            long dist = calc_dist(g1[0],g1[1],g2[0],g2[1],empty_rows,empty_cols);
            res += dist;
        }
    }
    cout << "res " << res<< endl;
