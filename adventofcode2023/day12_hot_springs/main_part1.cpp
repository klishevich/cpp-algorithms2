...
int calc(const string &spr, const vector<int> &segms) {
    ...
    if (segms.size() == 1) {
        int res = 0;
        for (int i=0;i<spr_len - segms[0] + 1;i++) {
            if (no_hash(spr,0,i) && no_dot(spr, i, segms[0]) && no_hash(spr,i+segms[0],spr_len-(i+segms[0]))) res++;
        }
        return res;
    }
    int res = 0;
    ...
    for (int i=0;i<spr_len;i++) {
        ...
        if (no_hash(spr,0,i) && no_dot(spr,i,cur_segm_len) && no_hash(spr,i+cur_segm_len,1)) {
